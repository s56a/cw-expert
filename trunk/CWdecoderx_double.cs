...
namespace CWExpert
{
    public class CWDecode
    {
        ...
        public float[,] Mag = new float[32, FFTlen];
...
/*                    Marshal.Copy(audio_buffer, 0, fftw.pin, Audio.BlockSize);
        ...
        private void FFT_Spectrum()
        {
            try
            {
                ...
                while (z < nofs - 1)  // nofs iterations including first unique one
                {
                    ...
                    Marshal.Copy(RealF, 0, fftw.pin, F2L);

                    if (fftw.ComputeFFT())
                    {
                        Array.Copy(fftw.fout, 0, RealF, 0, F2L);
                    }
                    ...
                    {
//                        byte y = bitrev[n];
                        Mag[z, n] = Math.Abs(RealF[n]); // Median(Math.Sqrt((RealF[y] * RealF[y]) + (ImagF[y] * ImagF[y])), n);
